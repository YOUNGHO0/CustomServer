...
# Java를 사용해 서버를 직접 만들고 부하테스트를 주면서 개선하기 
...
## 1-2 처리 방식에 따라 동기식 처리 클래스, 비동기(스레드 풀 활용 클래스로 나뉘어짐)
...
## 2-7 DB부하가 많이 걸려있는 상황에서는 커넥션 풀 갯수가 크게 영향을 주는 요소는 아닌 것으로 보였음
DB 테이블에 100만개의 열을 집어 넣고, Count 쿼리를 발생시킴
인덱스가 걸려있었지만 쿼리 수행 속도는 약 0.72ms 정도였음 (인덱스를 잘못 걸었을 수도 있음 DB의 더 깊은 공부 필요 RealMySql을 읽어라!)
(내용추가 필요)
